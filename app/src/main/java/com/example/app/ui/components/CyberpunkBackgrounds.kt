...
import com.example.app.ui.theme.NeonCyan
        ...
        for (row in -1 until rows) {
            for (col in -1 until cols) {
                ...
                if (offsetX < -hexSize || offsetX > width + hexSize || 
                    offsetY < -hexSize || offsetY > height + hexSize) continue
...
fun DigitalLandscapeBackground(
    ...
    Canvas(modifier = modifier.fillMaxSize()) {
        ...
        val gridSpacing = width / (gridLineCount - 1)
        ...
        for (i in 0 until gridLineCount) {
            ...
            val startX = width * 0.5f * (1 - perspectiveFactor) - width * 0.1f * perspectiveShift * perspectiveFactor
            val endX = width - width * 0.5f * (1 - perspectiveFactor) + width * 0.1f * perspectiveShift * perspectiveFactor
